...
`results_load_wait_ms: 4000` 是结果候选的最大自适应等待时间，不再是固定暂停。程序每隔 `results_poll_interval_ms: 100` 检查一次 Top-N 图片锚点，数量达到要求并连续稳定后立即解析。`search_parse_timeout_ms: 5000` 约束进入首页、等待搜索框、输入关键词、提交导航与域名解析，不计入该自适应结果等待；解析阶段使用剩余预算。控制台分别显示“首页 / 搜索框 / 输入 / 提交 / 结果加载 / 解析”耗时和具体超时阶段。首次首页、搜索框或输入阶段发生瞬时超时时，程序会在同一份总预算内回退为直接搜索 URL 一次；导航已耗尽预算、结果加载不足或解析超时时不重复发起搜索，而是标记后继续。正式任务与诊断测试共用图片结果卡片结构化解析器，不再扫描页面全部外链，也不会按重复来源 URL 压缩排名。
