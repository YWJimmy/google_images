...
网络出口区域的“可用节点延迟采集”会读取所选 Clash Selector 组，递归展开实际代理节点，并通过 Mihomo 固定 HTTP 探测接口并发检测可用性和延迟。检测不会切换当前节点，结果只显示在当前页面，不保存服务器地址、完整出口 IP、订阅信息或凭据。这里的延迟是代理 HTTP 往返延迟，不是容易被服务器禁用的 ICMP ping。

测速后，节点下拉选项会同时显示最近延迟和该节点上次触发人工验证的时间。节点归属从本版本之后的新验证记录开始保存；旧记录没有节点标识，不做推断。若多个 Chrome 都配置同一个 Clash 本机端口，它们仍共享当前 Selector；要获得互相独立的出口，需要在 Mihomo 中为不同节点或策略组建立不同的回环 listener 端口，再把这些端口分别配置给对应 Chrome。
